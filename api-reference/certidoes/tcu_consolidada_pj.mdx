---
title: "TCU — Consulta Consolidada (PJ)"
api: "POST /v1/certidoes/tcu_consolidada_pj"
description: "Consulta consolidada de impedimentos e ocorrências no Tribunal de Contas da União para Pessoa Jurídica."
---

## Descrição

Consulta consolidada de impedimentos e ocorrências no Tribunal de Contas da União para Pessoa Jurídica.

A consulta é **síncrona**: a API processa e retorna os dados estruturados e,
quando houver, os arquivos oficiais (PDF) com **URL de download assinada
(validade de 30 minutos)**.

## Autenticação

Envie sua API key no header `Authorization: Bearer SEU_TOKEN`. Cada chamada
bem-sucedida consome **1 unidade** da cota mensal. Em erro de consulta a cota
é estornada.

## Parâmetros

<ParamField body="cnpj" type="string" required>
  CNPJ do titular (apenas dígitos, 14 caracteres).
</ParamField>

## Resposta

<ResponseField name="certidao" type="string">
  Identificador da certidão consultada (`tcu_consolidada_pj`).
</ResponseField>

<ResponseField name="data" type="object">
  Payload estruturado do resultado (varia conforme a certidão).
</ResponseField>

<ResponseField name="arquivos" type="array">
  Lista de arquivos oficiais. Cada item traz `filename`, `content_type`,
  `kind`, `url` (assinada) e `expires_in` (segundos até expirar — 1800 = 30 min).
</ResponseField>

<RequestExample>

```bash cURL
curl -X POST "https://data.dadosfazenda.com.br/v1/certidoes/tcu_consolidada_pj" \
  -H "Authorization: Bearer SEU_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "cnpj": "12345678000190"
  }'
```

</RequestExample>

<ResponseExample>

```json 200 OK
{
  "success": true,
  "message": "Success",
  "data": {
    "certidao": "tcu_consolidada_pj",
    "data": { },
    "arquivos": [
      {
        "filename": "tcu_consolidada_pj.pdf",
        "content_type": "application/pdf",
        "kind": "official_pdf",
        "url": "https://data.dadosfazenda.com.br/storage/v1/object/sign/certidoes/...",
        "expires_in": 1800
      }
    ]
  }
}
```

```json 401 — API key inválida
{ "detail": "API key invalida." }
```

```json 429 — cota mensal atingida
{ "detail": "Limite mensal atingido. Reseta no proximo mes." }
```

</ResponseExample>
